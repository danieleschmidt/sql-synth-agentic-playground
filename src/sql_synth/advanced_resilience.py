...
class AdaptiveRetry:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if not self._should_retry(failure_type, attempt):
                    logger.error(f"Not retrying {operation_id}, failure type: {failure_type.value}")
                    break
    ...
    def _classify_failure(self, exception: Exception) -> FailureType:
        ...
        elif 'auth' in exception_msg or 'permission' in exception_msg:
            return FailureType.AUTHENTICATION_ERROR
        elif 'rate' in exception_msg or 'limit' in exception_msg:
...
class BulkheadIsolation:
    ...
    def execute_isolated(self, func: Callable, pool_name: str = "default", *args, **kwargs) -> Any:
        ...
        if metrics['active_tasks'] >= self.config.bulkhead_max_concurrent:
            raise ResourceExhaustedException(f"Pool {pool_name} at capacity")
