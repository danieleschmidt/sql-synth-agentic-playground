...
class ApplicationMonitor:
    ...
    def export_prometheus_metrics(self) -> str:
        ...
        # Query metrics
        prometheus_output.append("# HELP sql_synth_query_total Total number of queries processed")
        prometheus_output.append("# TYPE sql_synth_query_total counter")
        prometheus_output.append(f"sql_synth_query_total {metrics['query_metrics']['total_queries']}")

        prometheus_output.append("# HELP sql_synth_query_duration_seconds Average query duration")
        prometheus_output.append("# TYPE sql_synth_query_duration_seconds gauge")
        prometheus_output.append(f"sql_synth_query_duration_seconds {metrics['query_metrics']['average_time']}")

        prometheus_output.append("# HELP sql_synth_query_duration_p95_seconds 95th percentile query duration")
        prometheus_output.append("# TYPE sql_synth_query_duration_p95_seconds gauge")
        ...
        for error_type, count in metrics["error_metrics"].items():
            prometheus_output.append("# HELP sql_synth_errors_total Total number of errors by type")
            prometheus_output.append("# TYPE sql_synth_errors_total counter")
            prometheus_output.append(f'sql_synth_errors_total{{error_type="{error_type}"}} {count}')

        # System metrics
        prometheus_output.append("# HELP sql_synth_memory_usage_percent Memory usage percentage")
        prometheus_output.append("# TYPE sql_synth_memory_usage_percent gauge")
        prometheus_output.append(f"sql_synth_memory_usage_percent {metrics['system_metrics']['memory']['usage_percent']}")

        prometheus_output.append("# HELP sql_synth_disk_usage_percent Disk usage percentage")
        prometheus_output.append("# TYPE sql_synth_disk_usage_percent gauge")
