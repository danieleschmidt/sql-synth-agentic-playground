...
class VisualQueryParser:
    ...
    def parse_visual_input(self, image_data: Union[str, bytes, Image.Image]) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(image_data, str):
                ...
                msg = f"Unsupported image format: {type(image_data)}"
            ...
            logger.exception(f"Visual parsing failed: {e}")
            return {
                "error": str(e),
                "visual_type": "unknown",
                "confidence": 0.0,
    ...
    def _detect_chart_type(self, image: Image.Image, features: dict[str, Any]) -> str:
        ...
        if 0.8 <= aspect_ratio <= 1.2 and complexity > 0.3:
            return "pie_chart"
        if aspect_ratio > 1.5 and complexity > 0.2:
            return "bar_chart"
        if aspect_ratio > 1.2 and complexity > 0.15:
            return "line_chart"
        if complexity > 0.4:
            return "dashboard"
        if complexity < 0.1:
...
class ContextualFusionEngine:
    ...
    def fuse_multi_modal_inputs(
        ...
        try:
            ...
            logger.exception(f"Context fusion failed: {e}")
            return {
                "error": str(e),
                "fusion_strategy": strategy,
                "timestamp": time.time(),
    ...
    def _calculate_ensemble_consensus(
        ...
        total_weight = sum(voting_summary[mod]["vote_weight"] for mod in modality_values)
        if total_weight > 0:
            consensus_score = sum(
                voting_summary[mod]["vote_weight"] * voting_summary[mod]["confidence"]
                for mod in modality_values
            ) / total_weight
    ...
    def _calculate_fusion_confidence(self, inputs: list[ModalInput], fusion_result: dict[str, Any]) -> float:
        ...
        unique_modalities = len({inp.modality for inp in inputs})
        diversity_bonus = min(unique_modalities * 0.05, 0.15)  # Up to 15% for diverse inputs

        return min(base_confidence + confidence_adjustment + diversity_bonus, 1.0)
...
class MultiModalSQLSynthesizer:
    ...
    def synthesize_from_multimodal_input(
        ...
        try:
            ...
            if "error" in fusion_result:
                msg = f"Fusion failed: {fusion_result['error']}"
                raise ValueError(msg)
            ...
            if not base_result.get("success", False):
                msg = f"Base SQL generation failed: {base_result.get('error', 'Unknown error')}"
                raise ValueError(msg)
            ...
            synthesis_time = time.time() - start_time
            logger.exception(f"Multi-modal SQL synthesis failed: {e}")
    ...
    def _extract_visual_hints(self, fused_representation: dict[str, Any]) -> list[str]:
        ...
        for _key, values in fused_content.items():
